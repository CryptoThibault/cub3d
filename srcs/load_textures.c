...
/*   Updated: 2024/11/19 18:02:14 by achevron         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_textures(t_data *data)
{
	data->textures[0] = mlx_xpm_file_to_image(data->mlx_ptr,
			data->north_texture, &data->tex_size[0].x, &data->tex_size[0].y);
	if (!data->textures[0])
		perror_exit("failed to load north texture", data);
	data->textures[1] = mlx_xpm_file_to_image(data->mlx_ptr,
			data->south_texture, &data->tex_size[1].x, &data->tex_size[1].y);
	if (!data->textures[1])
